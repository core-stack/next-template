import { env } from "@/env";
import { getWorkspace, hasAccess } from "@/lib/utils";
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const { email, role } of input.emails) {
        ...
        let invite = await prisma.invite.findUnique({
          where: { workspaceId_email: { email, workspaceId: workspace.id } },
          include: { role: true }
        });
        if (invite) {
          ...
          prisma.$transaction(async (tx) => {
            const invite = await tx.invite.create({
              data: {
                ...
                user: { connect: { id: userWithEmail.id } },
                role: { connect: { id: role } },
                creator: { connect: { id: getWorkspace(ctx.session, workspace.slug)!.memberId } },
              },
              include: { role: true }
            });
            await addInQueue(QueueName.EMAIL, {
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await prisma.member.create({
        data: {
          ...
          user: { connect: { email: invite.email } }
        }
      })
  ...
  delete: rbacProcedure([Permission.DELETE_INVITE])
    ...
    .mutation(async ({ input }) => {
      await prisma.$transaction(async (tx) => {
        await tx.invite.delete({ where: { id: input.id } });
        await tx.notification.deleteMany({ where: { link: `/invite?code=${input.id}` } });
      })
    })
});